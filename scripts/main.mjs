...
[inputStartGeneratorProduct, inputStartDate].forEach((input) =>
...
[inputWeeks, inputWeeklyProducts, inputDaysInaWeek, inputCheckTreshold].forEach((input) =>
  input.addEventListener("keydown", (e) => {
    if (
      e.key.match(/[^-\d]/g) &&
      e.key !== "Backspace" &&
...
inputStartGeneratorProduct.addEventListener("input", (e) =>
  validator.checkStartDate(inputStartGeneratorProduct, e, cnf.maxLenghtDate, regexInputStartDate, inputs, inputSave)
);
inputStartDate.addEventListener("input", (e) =>
  validator.checkStartDate(inputStartDate, e, cnf.maxLenghtDate, regexInputStartDate, inputs, inputSave)
);
inputWeeks.addEventListener("input", (e) => validator.checkOtherInputs(inputWeeks, e, cnf.maxWeeks, inputs, inputSave));
inputWeeklyProducts.addEventListener("input", (e) =>
  validator.checkOtherInputs(inputWeeklyProducts, e, cnf.maxWeeklyProducts, inputs, inputSave)
);
inputDaysInaWeek.addEventListener("input", (e) =>
  validator.checkOtherInputs(inputDaysInaWeek, e, cnf.maxDaysInaWeek, inputs, inputSave)
);
inputCheckTreshold.addEventListener("input", (e) =>
  validator.checkOtherInputs(inputCheckTreshold, e, cnf.maxTreshold, inputs, inputSave)
);
