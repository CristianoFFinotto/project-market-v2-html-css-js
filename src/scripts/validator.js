 ...
 * @param {number} maxLenght - max number of char
 * @param {object} regex - regular expression
 * @param {array} inputs - all inputs form except save and reset
 * @param {object} save - save input DOM
 */
 ...
 * @param {object} inputs - all inputs form except save and reset
 * @param {object} save - save input DOM
 */
...
function totalCheck(inputs, save) {
  if (
    inputs.every(
      (input) => input.classList.contains("valid-input") && !input.classList.contains("error-input")
    )
  )
    save.disabled = false;
