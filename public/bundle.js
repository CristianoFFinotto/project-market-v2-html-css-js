...
;// CONCATENATED MODULE: ./src/scripts/functions.js
...
var generateName = function generateName(itemNames) {
  var randomIndex = Math.floor(Math.random() * itemNames.length);
...
function generateItems(numberOfItems, itemNames, startExpiry, endDate, currentDate, shelfLife) {
  var weeklyItems = [];
...
function updateState(item, currentDate, shelfLife) {
...
function createTable(itemArray, node) {
  ...
  var tableRowThead = document.createElement("tr");
  ...
  var thExpiry = document.createElement("th");
  thExpiry.textContent = "Expiry Date";
  tableRowThead.appendChild(thExpiry);
  var thChecks = document.createElement("th");
  ...
  itemArray.forEach(function (item) {
    var tableRowItem = document.createElement("tr");
...
function printContent(itemArray, currentDate, sectionId, nodeContent) {
  var sectionContent = document.createElement("div");
  sectionContent.id = sectionId;

  if (sectionContent.id < config.weeksRuntime) {
  ...
  title.textContent = "Date: ".concat(currentDate.toLocaleDateString());
  ...
  var filtered = document.createElement("h2");
  ...
  buttonNext.onclick = function () {
    var nextSection = document.getElementById(sectionId - 1);
    sectionContent.classList.add("d-none");
  ...
  buttonPrevious.onclick = function () {
    if (sectionId + 1 <= config.weeksRuntime) {
      var previousSection = document.getElementById(sectionId + 1);
      sectionContent.classList.add("d-none");
      previousSection.classList.remove("d-none");
    }
  };

  if (sectionId === config.weeksRuntime) {
...
function openCloseMenu() {
  ...
  var button = document.getElementById("setting-btn");
...
;// CONCATENATED MODULE: ./src/scripts/itemsNames.js
/**
 * @file itemNames.mjs
 * @authors Davide Murroni, Simone Sporeni, Paolo Gippa, Cristiano Francesco Finotto
 * File containing an array of all possible item names.
 ...
 * when generating a random item for the market.
 */
var itemNames = ["Asparagus", "Apples", "Avacado", "Alfalfa", "Almond", "Arugala", "Artichoke", "Applesauce", "Antelope", "Albacore Tuna", "Apple Juice", "Avocado Roll", "Bruschetta", "Bacon", "Black Beans", "Bagels", "Baked Beans", "Bbq", "Bison", "Barley", "Beer", "Bisque", "Bluefish", "Bread", "Broccoli", "Burrito", "Babaganoosh", "Cabbage", "Cake", "Carrots", "Carne Asada", "Celery", "Cheese", "Chicken", "Catfish", "Chips", "Chocolate", "Chowder", "Clams", "Coffee", "Cookies", "Corn", "Cupcakes", "Crab", "Curry", "Cereal", "Chimichanga", "Dates", "Dips", "Duck", "Dumplings", "Donuts", "Eggs", "Enchilada", "Eggrolls", "English Muffins", "Edimame", "Eel Sushi", "Fajita", "Falafel", "Fish", "Franks", "Fondue", "French Toast", "French Dip", "Garlic", "Ginger", "Gnocchi", "Goose", "Granola", "Grapes", "Green Beans", "Guancamole", "Gumbo", "Grits", "Graham Crackers", "Ham", "Halibut", "Hamburger", "Honey", "Huenos Rancheros", "Hash Browns", "Hot Dogs", "Haiku Roll", "Hummus", "Ice Cream", "Irish Stew", "Indian Food", "Italian Bread", "Jambalaya", "Jelly", "Jerky", "Jalapeño", "Kale", "Kabobs", "Ketchup", "Kiwi", "Kidney Beans", "Kingfish", "Lobster", "Lamb", "Linguine", "Lasagna", "Meatballs", "Moose", "Milk", "Milkshake", "Noodles", "Oreo", "Pizza", "Pepperoni", "Porter", "Pancakes", "Quesadilla", "Quiche", "Reuben", "Spinach", "Spaghetti", "Tater Tots", "Toast", "Venison", "Waffles", "Wine", "Walnuts", "Yogurt", "Ziti", "Zucchini"];
;// CONCATENATED MODULE: ./src/scripts/validator.js
 ...
 */
function checkStartDate(input, e, maxLenght, regex, inputs, save) {
...
function checkOtherInputs(input, e, maxValue, inputs, save) {
  var inputValue = e.target.value;
...
function totalCheck(inputs, save) {
  if (inputs.every(function (input) {
    return input.classList.contains('valid-input') && !input.classList.contains('error-input');
  })) save.disabled = false;else save.disabled = true;
}
;// CONCATENATED MODULE: ./src/img/logo-market-inventory-system.jpg
/* harmony default export */ const logo_market_inventory_system = (__webpack_require__.p + "./img/logo-market-inventory-system.jpg");
 ...
 */
 ...
 //main functions used to run the program

 //array with a list of all possible item names
...
var inputStartGeneratorProduct = document.querySelector('#start-generator-product');
var inputStartDate = document.querySelector('#start-date');
var inputWeeks = document.querySelector('#weeks');
var inputWeeklyProducts = document.querySelector('#weekly-products');
var inputDaysInaWeek = document.querySelector('#days-in-week');
var inputCheckTreshold = document.querySelector('#check-threshold');
var inputSave = document.querySelector('#save');
var inputReset = document.querySelector('#reset');
var inputSetting = document.querySelector('#setting-btn');
/********************** REGEX **********************/

var regexInputStartDate = new RegExp('(0[1-9]|1[0-2])\/(0[1-9]|[1-3][0-9])\/[1-9][0-9]{3}');
var inputs = [inputStartGeneratorProduct, inputStartDate, inputWeeks, inputWeeklyProducts, inputDaysInaWeek, inputCheckTreshold];
/********************** disable paste into inputs **********************/

inputs.forEach(function (input) {
  return input.addEventListener('paste', function (e) {
...
[inputStartGeneratorProduct, inputStartDate].forEach(function (input) {
  return input.addEventListener('keydown', function (e) {
    if (e.key.match(/[^\/\d]/g) && e.key !== 'Backspace' && e.key !== 'Tab' && e.key !== 'ArrowUp' && e.key !== 'ArrowDown' && e.key !== 'ArrowLeft' && e.key !== 'ArrowRight') e.preventDefault();
  });
});
/********************** permit insert number backspace tab arrow t,r,b,l minus **********************/

[inputWeeks, inputWeeklyProducts, inputDaysInaWeek, inputCheckTreshold].forEach(function (input) {
  return input.addEventListener('keydown', function (e) {
    if (e.key.match(/[^-\d]/g) && e.key !== 'Backspace' && e.key !== 'Tab' && e.key !== 'ArrowUp' && e.key !== 'ArrowDown' && e.key !== 'ArrowLeft' && e.key !== 'ArrowRight') e.preventDefault();
  });
});
...
inputStartGeneratorProduct.addEventListener('input', function (e) {
  return checkStartDate(inputStartGeneratorProduct, e, config.maxLenghtDate, regexInputStartDate, inputs, inputSave);
});
inputStartDate.addEventListener('input', function (e) {
  return checkStartDate(inputStartDate, e, config.maxLenghtDate, regexInputStartDate, inputs, inputSave);
});
inputWeeks.addEventListener('input', function (e) {
  return checkOtherInputs(inputWeeks, e, config.maxWeeks, inputs, inputSave);
});
inputWeeklyProducts.addEventListener('input', function (e) {
  return checkOtherInputs(inputWeeklyProducts, e, config.maxWeeklyProducts, inputs, inputSave);
});
inputDaysInaWeek.addEventListener('input', function (e) {
  return checkOtherInputs(inputDaysInaWeek, e, config.maxDaysInaWeek, inputs, inputSave);
});
inputCheckTreshold.addEventListener('input', function (e) {
  return checkOtherInputs(inputCheckTreshold, e, config.maxTreshold, inputs, inputSave);
});
inputSetting.addEventListener('click', function () {
  return openCloseMenu();
});
inputSave.addEventListener('click', function (e) {
  e.preventDefault();
  config.startGeneratorExpiring = inputStartGeneratorProduct.value;
  config.startProgramDate = inputStartDate.value;
  config.weeksRuntime = Number(inputWeeks.value);
  config.newItemsPerWeek = Number(inputWeeklyProducts.value);
  config.daysInWeek = Number(inputDaysInaWeek.value);
  config.shelfLife = Number(inputCheckTreshold.value);
  config.id = 1;
  openCloseMenu();
  init();
});
...
inputReset.addEventListener('click', function () {
  inputs.forEach(function (input) {
    return input.classList.remove('valid-input', 'error-input');
  });
...
function init() {
  var runtime = config.weeksRuntime;
  ...
  var currentDate = new Date(config.startProgramDate);
  var endDate = addDays(currentDate, runtime * config.daysInWeek);
  var startExpiry = new Date(config.startGeneratorExpiring);
  var shelfLife = config.shelfLife;

  for (var i = runtime; i > 0; i--) {
    var _items;

    /* 1) Add new items */
    (_items = items).push.apply(_items, _toConsumableArray(generateItems(config.newItemsPerWeek, itemNames, endDate, startExpiry, currentDate, shelfLife)));

    printContent(items, new Date(currentDate), i, nodeContent);
    /* 2) Filter the items */

    items = items.filter(checkItem);
    /* 4) Add days to the current date */

    currentDate = addDays(currentDate, config.daysInWeek);
    /* 5) Update the items (state and checks) */

    items.forEach(function (item) {
      updateChecks(item);
      updateState(item, currentDate, config.shelfLife);
    });
  }
...
;
var orderedItems = (/* unused pure expression or super */ null && ([]));

var sortTable = function sortTable(emptyArray, itemArray) {
  itemArray.forEach(function (item) {
